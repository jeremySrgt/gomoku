...
v1.3 : création fonction blocage pour la base de l'arbre des possibilités

v1.4 : amélioration des détections des diagonales
							...
							if (not blocageApres[0]):
								...
								if (longueurMax in tailleBlocage):
									...
									if (not blocageAvant[0]):
										...
										serieBlocage[blocageAvant[1]]=-10**(longueurMax+3)
										serieBlocage[blocageApres[1]]=-10**(longueurMax+3)
...
def IA(tourTMP, profondeur, matriceTMP, abrTMP):
	...
	if (profondeurVictoire>profondeur>=0 and abrTMP.getCanExpand()):
		...
		for possibilite in dictionnaireTMP.keys():
			...
			if (abs(1-profondeur)>=profondeurVictoire):
				canExpandTMP = False
			else:
				canExpandTMP = True
			...
			if (victoireTMP and (tour+tourTMP)%2==0):
				if (abs(1-profondeur)<profondeurVictoire):
					profondeurVictoire = abs(1-profondeur)
					canExpandTMP = False

			abrTMP.ajoutFils(ABR({possibilite:poids},[], canExpandTMP))
...
def calculPoids(seriesJoueur1TMP, seriesJoueur2TMP, victoireTMP, tourTMP):
	...
	for indice in range(2,nbVictoire):
		poids += (10**indice)*(seriesJoueur1TMP[indice]+seriesJoueur2TMP[indice])
...
def main():
    ...
    if ( tour >= (2*nbVictoire-1)):
    	victoire, serieJoueur1TMP, serieJoueur2TMP, serieBlocageTMP = testVictoire(matriceJeu, False)
    	if ( victoire ):
    		return
